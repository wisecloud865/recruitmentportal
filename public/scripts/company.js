    ...
    try {
        ...
        data.forEach(company => {
            ...
            if (company.Ekonomisiffor) {
                ...
                omsattningRow.innerHTML = `<td class="info-label">Omsättning</td><td class="info-value">${company.Ekonomisiffor.Omsättning || 'N/A'}</td>`;
                tbody.appendChild(omsattningRow);

                const resultatRow = document.createElement('tr');
                resultatRow.innerHTML = `<td class="info-label">Resultat</td><td class="info-value">${company.Ekonomisiffor.Resultat || 'N/A'}</td>`;
                tbody.appendChild(resultatRow);

                const omsattningsforandringRow = document.createElement('tr');
                omsattningsforandringRow.innerHTML = `<td class="info-label">Omsättningsförändring</td><td class="info-value">${company.Ekonomisiffor.Omsättningsförändring || 'N/A'}</td>`;
        ...
        companySection.innerHTML = `<p class="error-message">Failed to load company data. Please try again later.</p>`;
    }
});
...
function addDropdownFunctionality(container, table) {
    ...
    dropdownBtn.addEventListener('click', function() {
        ...
        dropdownBtn.innerHTML = isExpanded ? 'Show Less <i class="fas fa-caret-up"></i>' : 'Show More <i class="fas fa-caret-down"></i>';
