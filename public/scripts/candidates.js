document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        data.forEach((company, index) => {
            ...
            if (company.matched_candidates && company.matched_candidates.length > 0) {
                ...
                company.matched_candidates.forEach((candidate, candidateIndex) => {
                    ...
                    candidateTitle.innerHTML = `<i class="fas fa-user"></i> ${candidate.full_name || `Candidate ${candidateIndex + 1}`}`;
                    ...
                    const defaultFields = ['total_workexperience', 'techterms', 'current_company'];
                    
                    // Sort entries to show default fields first
                    const sortedEntries = Object.entries(candidate).sort(([keyA], [keyB]) => {
                    ...
                    sortedEntries.forEach(([key, value], rowIndex) => {
                        ...
                        row.innerHTML = `
                            ...
                            <td class="info-value">${typeof value === 'object' ? JSON.stringify(value) : value}</td>
                        `;
        ...
        candidatesSection.innerHTML = `<p class="error-message">Failed to load data. Please try again later.</p>`;
    }
});
